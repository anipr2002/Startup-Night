...
function DialogContent({ children, className, style }: DialogContent) {
  ...
  return (
    <motion.div
      ref={containerRef}
      layoutId={`dialog-${uniqueId}`}
      className={cn("overflow-hidden font-roger", className)}
      style={style}
      role="dialog"
      aria-modal="true"
      ...
      {isOpen && (
        <>
          <motion.div
            ...
            className="fixed inset-0 h-full w-full bg-white/40 backdrop-blur-sm dark:bg-black/40 font-roger"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          />
          <div className="fixed inset-0 z-50 flex items-center justify-center font-roger">
            {children}
          </div>
        </>
      )}
...
function DialogTitle({ children, className, style }: DialogTitleProps) {
  ...
  return (
    <motion.div
      ...
      className={cn("font-roger", className)}
      style={style}
      layout
    >
      {children}

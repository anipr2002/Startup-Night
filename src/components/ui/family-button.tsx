...
const FamilyButton: React.FC<FamilyButtonProps> = ({ children }) => {
  const { shoppingListOpen, inventroyOpen } = useActionState();
  ...
  return (
    <div
      className={cn(
        "rounded-[24px] border shadow-sm transition-opacity delay-300 ease-linear duration-300",
        "bg-gradient-to-b  from-neutral-900 to-black",
        isExpanded
          ? "w-[150px] bg-gradient-to-b dark:from-stone-900 dark:to-neutral-900/80"
          : "dark:from-neutral-900 dark:to-stone-950 bg-gradient-to-b",

        shoppingListOpen && "opacity-0 transition-none",
        inventroyOpen && "opacity-0 transition-none"
      )}
    >
  ...
  return (
    ...
    >
      ...
      >
        {isExpanded ? (
          <motion.div
            className="p-[10px] group bg-neutral-800/50 dark:bg-red-500/50 border border-cyan-100/30 hover:border-neutral-200 text-orange-50 rounded-full shadow-2xl transition-colors duration-300 "
          ...
          <motion.div
            className={cn(
              "p-[10px] group bg-neutral-200 dark:bg-neutral-500/90 text-cyan-50 border border-cyan-100/10  shadow-2xl transition-colors duration-200"
            )}
